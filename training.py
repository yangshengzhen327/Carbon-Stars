...
def stratified_group_split_indices(labels, groups, val_ratio=0.2, seed=42):
    """Split sample indices while keeping every astronomical object intact."""
    ...
    groups_by_label = {}
    for group, member_indices in group_to_indices.items():
        member_labels = np.unique(labels[member_indices])
        if len(member_labels) != 1:
            raise RuntimeError(
                f"Object {group!r} occurs under multiple auxiliary classes: {member_labels.tolist()}"
            )
        groups_by_label.setdefault(member_labels[0], []).append((group, member_indices))

    rng = np.random.RandomState(seed)
    val_groups = set()
    for label, label_groups in sorted(groups_by_label.items(), key=lambda item: str(item[0])):
        if len(label_groups) < 2:
            raise RuntimeError(
                f"Auxiliary class {label!r} has only {len(label_groups)} unique object group(s); "
                "at least two are required for a leakage-free train/validation split."
            )
        shuffled = list(label_groups)
        rng.shuffle(shuffled)
        target_samples = sum(len(indices) for _, indices in shuffled) * float(val_ratio)
        selected_samples = 0
        for position, (group, member_indices) in enumerate(shuffled):
            groups_left = len(shuffled) - position
            if groups_left <= 1:
                break
            current_error = abs(selected_samples - target_samples)
            added_error = abs(selected_samples + len(member_indices) - target_samples)
            if selected_samples == 0 or added_error <= current_error:
                val_groups.add(group)
                selected_samples += len(member_indices)
            elif selected_samples >= target_samples:
                break
    ...
    if len(train_idx) == 0 or len(val_idx) == 0:
        raise RuntimeError("Group split produced an empty train or validation subset")
    overlap = set(groups[train_idx]).intersection(set(groups[val_idx]))
    if overlap:
        raise AssertionError(f"Object-level split leakage detected for {len(overlap)} group(s)")
...
def build_selection_key(select_by, val_metrics, metric_targets=None):
...
def main(argv=None):
    ...
    parser.add_argument(
        "--select_by",
        type=str,
        default=DEFAULT_SELECT_BY,
        choices=["val_accuracy", "val_f1", "val_macro", "val_benchmark"],
        help="Validation metric used for checkpoint selection. The test set is never used for selection.",
    )
    ...
    print("[INFO] threshold_source = val (fixed)")
    ...
    object_groups = [sample["object_id"] for sample in trainval_samples]
    split_labels = y_all_aux if args.split_stratify == "aux" else y_all
    ...
    print(f"[INFO] train unique objects = {len({sample['object_id'] for sample in train_samples})}")
    print(f"[INFO] val unique objects   = {len({sample['object_id'] for sample in val_samples})}")
    ...
    if args.model_name == "cspec_db_net":
        model = CSpecDBNet(
            signal_len=N_PIX,
        ...
        model = create_baseline_model(args.model_name, signal_len=N_PIX, **args.model_kwargs).to(device)
    print(
        f"[INFO] index_feature_dim = {len(model.index_feature_names)} "
        f"(branch_in={model.index_feature_branch_input_dim})"
    )
    index_feature_full_names = tuple(getattr(model, "index_feature_full_names", ()))
    index_feature_names = tuple(getattr(model, "index_feature_names", ()))
    index_feature_dropped_names = tuple(getattr(model, "index_feature_dropped_names", ()))
    index_feature_branch_input_dim = int(getattr(model, "index_feature_branch_input_dim", 0))
